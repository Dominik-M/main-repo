package galculator;

import eds.DoubleStack;


public class Galculator{
  private static DoubleStack stack= new DoubleStack();
  
  public Galculator(String s){
    if(infix(s)) s=Transformer.toUPN(s);
    try{System.out.println(rechnen(s));}
    catch(NumberFormatException ex){System.out.println(s);}
    deleteAll();
  }
  
  public static double rechnen(String s){
    int l=s.length();
    char rech;
    for (int i=0;i<l;i++) {
      String teil="";
      while (s.charAt(i)!=' ' && i<l) {
        teil=teil+s.charAt(i);
        i++;
      }
      if(teil.charAt(0)+0<48){ // Da Rechenzeichen kleiner sind als 48, folgt jetzt sicherlich eine Rechnung
        rech=teil.charAt(0);
        if(rech=='+')stack.push(stack.pull()+stack.pull());//plus
        else if(rech=='-')stack.push(-stack.pull()+stack.pull());//minus
        else if(rech=='*')stack.push(stack.pull()*stack.pull());//mal
        else if(rech=='/'){
          double divident=stack.pull();
          stack.push(stack.pull()/divident);               //geteilt;
        }
      }
      else {
        stack.push(machZahl(teil));
      }
    }
    return stack.peak();
  }
  
  
  public static void deleteAll(){
//    double schrott=stack.wert;
//    while(stack.pull()!=0) schrott=stack.pull();
//    stack.wert=0;
  }
  
  public static double machZahl(String s){
    int t=s.indexOf(' ',0);
    if(t==-1) t=s.length();
    String zstr=s.substring(0,t);
    double zahl=Double.parseDouble(zstr);
    return zahl;
  }
  
  public static boolean infix(String s){
    return s.charAt(s.indexOf(' ',0)+1)+0<48;
    //wenn nach dem ersten Leerzeichen (nach der ersten Zahl) noch eine Zahl kommt ist es höchstwahrscheinlich nicht Infix.
  }
}
