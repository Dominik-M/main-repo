...
public class Soundclip {
    ...
    public static final int MAX_MEDIA_INIT_WAIT_TIME = 100;
    ...
    private void initMedia() {
        ...
        int i = 0;
        while (mediaplayer.getStatus() != MediaPlayer.Status.READY && i < MAX_MEDIA_INIT_WAIT_TIME) {
            try {
                if (i % 10 == 0) {
                    System.out.println("Waiting for initialization of media " + media + " (" + sourcefile.getName() + ")");
                }
                sleep(50);
            } catch (InterruptedException ex) {
                Logger.getLogger(Soundclip.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        if (mediaplayer.getStatus() == MediaPlayer.Status.READY) {
            System.out.println("Init successful, MediaPlayer (" + mediaplayer + ") READY");
        } else {
            System.err.println("ERROR Timeout initializing Media " + media + " (" + sourcefile.getName() + ")");
        }
    }
