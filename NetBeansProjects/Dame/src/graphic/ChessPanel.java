...
public class ChessPanel extends MainPanel {
	...
	public int translateScreenX(int screenX) {
		return DameBoard.getInstance().getWidth() * screenX / getWidth();
	}

	public int translateScreenY(int screenY) {
		return DameBoard.getInstance().getHeight() * (screenY) / (getHeight());
	}

	public void selectFigureAt(int x, int y) {
		DameBoard board = DameBoard.getInstance();
		selX = x;
		selY = y;
		selFigure = board.getFigureAt(selX, selY);
		if (selFigure != null) {
			IO.println("Selected Figure: " + selFigure, IO.MessageType.DEBUG);
			movePositions = selFigure.getPossibleLocations(board);
		} else {
			removeSelection();
		}
	}
	...
	private void drawChessfield(Graphics2D g, Rectangle bounds) {
		DameBoard board = DameBoard.getInstance();
		int spotWidth = bounds.width / board.getWidth();
		int spotHeight = bounds.height / board.getHeight();
		...
		for (int x = 0; x < board.getWidth(); x++) {
			for (int y = 0; y < board.getHeight(); y++) {
				if (x == selX && y == selY) {
					...
					// } else if (board.getKing(Team.WEIS) != null
					// && board.getKing(Team.WEIS).isSchach(board) && board.getKing(Team.WEIS).getX() == x
					// && board.getKing(Team.WEIS).getY() == y) {
					// g.setColor(Color.red);
					// } else if (board.getKing(Team.SCHWARZ) != null
					// && board.getKing(Team.SCHWARZ).isSchach(board)
					// && board.getKing(Team.SCHWARZ).getX() == x
					// && board.getKing(Team.SCHWARZ).getY() == y) {
					// g.setColor(Color.red);
				} else if ((y + x) % 2 == 0) {
					g.setColor(BGCOLOR1);
				} else {
				...
				spotHeight);
				Figure fig = board.getFigureAt(x, y);
				if (fig != null) {
