...
/**
 * Diese Klasse stellt einen einfachen Zugriff auf die Midi-F?higkeiten
 * eines Rechners zur Verf?gung. Eine Tonfolge ist eine Menge von Noten
 ...
 * Weil eine Tonfolge mehrmals abspielbar sein soll, k?nnen diese Resourcen nach
 * dem ersten Abspielen nicht freigegeben werden. Ein Programm mit nicht beendeten
 * Threads endet aber nicht gew?hnlich, sondern muss mit <CODE>System.exit(0)</CODE>
 * beendet werden. Damit werden dann die beiden erw?hnten Threads auch get?tet.<p>
 * Wenn dieses (logische) Verhalten nicht gew?nscht wird, weil etwa eine Tonfolge
 ...
 * eingeschoben werden. Diese sorgt daf?r, dass die beiden Threads am Ende get?tet
 * werden. Die Tonfolge ist danach aber nicht weiter zu gebrauchen. Da solch ein
 * Verhalten un?blich ist, ist die Methode endNote eigentlich eine technische
...
public class Tonfolge implements MetaEventListener{
  ...
  public void stop(){
    ...
    for(TonListener tl:tonListener)tl.ende();
  }
  ...
  /**
   * Diese Methode muss einmal am Ende des St?ckes aufgerufen werden, um
   ...
   * Eine Tonfolge bei Verwendung dieser Methode nur ein einziges Mal
   * abspielbar. Ein nachtr?gliches Bearbeiten der T?ne hat also keinen Sinn.
   ...
   * terminieren soll, dies aber auf Grund der hier erzeugten Threads dies nicht
   * tun w?rde.
   */
  public void endnote(){
   ...
   * mit der Musik nichts zu tun hat. Die Methode t?tet dann alle Threads. Ein
   * Programm, das eine <CODE>Tonfolge</CODE> verwendet, endet dann auf gew?hnlichem
   * Weg. Dies ist nur dann von Belang, wenn es sich um ein Konsolenprogramm handelt.
   * Programme mit GUI enden sowieso nicht gew?hnlich.
  ...
  public void meta(MetaMessage mm){
    ...
    for(int i=0; i<alle.length; i++)
      if(alle[i].getName().toLowerCase().indexOf("sound")>-1) alle[i].interrupt();
    for(TonListener tl:tonListener)tl.ende();
  }
