...
public class GamePanel extends javax.swing.JPanel implements MouseListener,MouseMotionListener{
    ...
    public GamePanel() {
        ...
        kartenviews = new bohnen.gui.KartenView[]{
            ablage,s0ablage,s0feld0,s0feld1,s0feld2,s0hand,
            s1ablage,s1feld0,s1feld1,s1feld2,s1hand,
            s2ablage,s2feld0,s2feld1,s2feld2,s2hand,stack
        };
    }
    ...
    public void karteMoved(Bohnenkarte karte,String from, String to){
        if(Bohnanza.DEBUG)
          System.out.println(this+": move "+karte+" from "+from+" to "+to);
    ...
    public void setBGColor(String pos, java.awt.Color color){
        KartenView view=findViewByName(pos);
        if(view!=null)view.setBackground(color);
    }
    ...
    public void paintComponent(Graphics g){
        ...
        for(KartenView k:kartenviews)
            k.paintComponent(g.create(k.getX(), k.getY(), k.getWidth(), k.getHeight()));
         if(selected!=null){
    ...
    private final bohnen.gui.KartenView[] kartenviews;
    @Override
