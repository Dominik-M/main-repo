...
public class MainFrame extends javax.swing.JFrame implements java.awt.event.WindowListener,
        java.awt.event.ComponentListener, template.utils.SettingsListener {
        ...
        public void actionPerformed(ActionEvent arg0) {
            for (int i = 0; i < languageMenuItems.length; i++) {
                if (languageMenuItems[i].equals(arg0.getSource())) {
                    template.utils.Text.switchLanguage(template.utils.Constants.Language.values()[i]);
                    break;
                }
            }
        }
    ...
    private MainFrame() {
        super(template.utils.Constants.APP_NAME);
        try {
            init();
        } catch (Exception ex) {
    ...
    private void init() {
        ...
        if (Constants.DEBUG_ENABLE) {
            ...
            setConsoleEnabled(Settings.getSettings().showConsole);
        }
        ...
        template.utils.Constants.Language[] langs = template.utils.Constants.Language.values();
        ...
        for (int i = 0; i < langs.length; i++) {
            ...
            if (langs[i] == template.utils.Text.getCurrentLanguage()) {
    ...
    public void setConsoleEnabled(boolean enabled) {
        ...
        Settings s = Settings.getSettings();
        s.showConsole = enabled;
        Settings.setSettings(s);
        if (!enabled && mainPanel != null) {
