...
public class Sound {
    ...
    public static boolean playRandomSound() {
        if (soundclips.size() > 0) {
            int i = (int) (Math.random() * soundclips.size());
            currentClip = soundclips.get(i);
            return play();
        }
        return false;
    }
    ...
    public static boolean addSoundFile(File file) {
        ...
        if (c != null) {
            if (soundclips.add(file, c)) {
                System.out.println("initialized soundfile: " + file.getName());
                return true;
            }
        }
        return false;
    ...
    public static void setMicrosecondPosition(long mys) {
        if (currentClip != null) {
            ...
            System.out.println("sound microsecond position set to " + mys);
        }
    }
...
//        Settings.setSettings(s);
    ...
    public static boolean playSoundClip(File file, int loops) {
        try {
            ...
            if (isSoundOn()) {
                ...
                System.out.println("Now playing sounfile: " + file.getName());
            }
        } catch (Exception ex) {
            System.out.println("Sound error: " + ex.toString());
            ex.printStackTrace();
            return false;
        }
        return true;
     ...
     *
     * @return Array of files of all initialised soundfiles.
     */
...
//        ProgressPanel.setText(Text.SEARCHING_SOUNDFILES.toString());
    ...
    public static File[] initClips(File[] files) {
        ...
        System.out.println("found soundfiles:");
        for (File f : files) {
...
//        ProgressPanel.setProgress(0);
        for (int i = 0; i < files.length; i++) {
            if (Sound.addSoundFile(files[i])) {
                ...
                System.out.println("Error while loading " + files[i]);
            }
...
//        ProgressPanel.setProgress(100);
    ...
    public static File[] listSoundFilesInDirectory(File directory) {
        if (!directory.exists() || !directory.isDirectory()) {
            System.out.println("Sound directory not found.");
            return null;
        }
