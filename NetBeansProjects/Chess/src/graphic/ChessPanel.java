	...
	private void drawChessfield(Graphics2D g, Rectangle bounds) {
		...
		for (int x = 0; x < game.getWidth(); x++) {
			for (int y = 0; y < game.getHeight(); y++) {
				...
				} else if (board.getKing(Chess.Team.WEIS) != null
				&& board.getKing(Chess.Team.WEIS).isSchach(board)
				&& board.getKing(Chess.Team.WEIS).getX() == x
				&& board.getKing(Chess.Team.WEIS).getY() == y) {
					g.setColor(Color.red);
				} else if (board.getKing(Chess.Team.SCHWARZ) != null
				&& board.getKing(Chess.Team.SCHWARZ).isSchach(board)
				&& board.getKing(Chess.Team.SCHWARZ).getX() == x
				&& board.getKing(Chess.Team.SCHWARZ).getY() == y) {
					g.setColor(Color.red);
				} else if ((y + x) % 2 == 0) {
					g.setColor(BGCOLOR1);
				} else {
				...
				spotHeight);
				Chess.Figure fig = board.getFigureAt(x, y);
				if (fig != null) {
