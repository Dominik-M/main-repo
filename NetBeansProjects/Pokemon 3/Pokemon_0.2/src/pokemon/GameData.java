...
public class GameData extends utils.SerializableReflectObject
{
    ...
    private static final LinkedList<GameDataListener> gameListener = new LinkedList<>();

    private Spieler player;
    private Trainer rivale;
    private ArrayList<Pokemon> box;
    private Dictionary<Text, Ort> world;
    private Ort pos;
    ...
    private void init()
    {
        ...
        rivale.setGeld(500);
        world = Ort.createWorld();
        pos = world.get(Text.ALABASTIA);
        player.setPos(pos.getStartX(), pos.getStartY());
    }
    ...
    public boolean switchOrt(int x, int y, Ort pos)
    {
        if (pos != null)
        {
            this.pos = pos;
            player.setPos(x, y);
            for (GameDataListener l : gameListener)
            {
                l.switchPos(pos);
            }
            return true;
    ...
    {
        Pokemon opponent = pos.getRandomPok();
        if (opponent != null)
        {
            Fight f = new Fight(player, opponent, true);
            for (GameDataListener l : gameListener)
            {
                l.startFight(f);
            }
