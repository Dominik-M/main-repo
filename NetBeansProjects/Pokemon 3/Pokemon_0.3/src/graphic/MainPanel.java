...
public abstract class MainPanel extends JPanel implements KeyListener, MouseListener
{
    ...
    {
        return decision != null && printQueue.size() <= Constants.OUTPUT_LINES;
    }

    void promptDecision(Decision d)
    {
        this.decision = d;
    }

    void promptItemDecision(DecisionCallback callback)
    {
        Decision d = new Decision(Constants.ITEM_MENU_BOUNDS, 10, 1, GameData.getCurrentGame().getPlayer().getItemStrings());
        d.setCallback(callback);
        this.decision = d;
    }

    /**
     * Pulls next texts from print queue.
     *
    ...
    private void drawOutput(Graphics2D g)
    {
        ...
        if (isDeciding())
        {
            drawFrame(g, decision.getBounds());
            g.setColor(Color.BLACK);
            int i = 0;
            for (int row = 0; row < decision.getRows(); row++)
            {
                for (int col = 0; col < decision.getCols(); col++)
                {
                    if (i < decision.getTexts().length)
                    {
                        String text = decision.getTexts()[i];
                        int x = decision.getBounds().x + decision.getBounds().width * col / decision.getCols() + Constants.BORDER_SIZE;
                        int y = decision.getBounds().y + decision.getBounds().height * row / decision.getRows() + Constants.BORDER_SIZE;
                        if (i == decision.getSelectedIndex())
                        {
                            int cY = y + Constants.SELECTION_MARKER_SIZE / 2;
                            g.fillPolygon(new java.awt.Polygon(new int[]
                            {
                                x, x, x + Constants.SELECTION_MARKER_SIZE
                            }, new int[]
                            {
                                cY + Constants.SELECTION_MARKER_SIZE / 2, cY - Constants.SELECTION_MARKER_SIZE / 2, cY
                            }, 3));
                        }
                        g.drawString(text, x + Constants.SELECTION_MARKER_SIZE + 5, y + 10);
                        i++;
                    }
                    else
                    {
    ...
    public static void drawTypImage(Graphics2D g, pokemon.Typ t, Rectangle bounds)
    {
        image.Sprite img = ImageIO.getSprite(t.name().toLowerCase() + "IC.png");
        if (img != null)
        {
            g.drawImage(img.getImage().getScaledInstance(bounds.width, bounds.height, 0), bounds.x, bounds.y, null);
        }
        else
        {
            g.drawString(t.toString(), bounds.x, bounds.y + bounds.height - 5);
        }
    }
}
