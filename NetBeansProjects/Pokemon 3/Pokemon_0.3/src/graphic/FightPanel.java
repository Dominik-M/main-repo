...
public class FightPanel extends MainPanel
{
    ...
    public final DecisionCallback ITEM_USE_IN_COMBAT = new DecisionCallback()
    {
        @Override
        public void decisionEntered(int index)
        {
            ...
            {
                sendAction(FightAction.ActionType.ITEM, index);
            }
        }
    };
     ...
     * ATTACK3   ATTACK4
     */
    private final InputOption ATTACK_1 = new InputOption("DEFAULT_TEXT", new Rectangle(Constants.OUTPUT_BOUNDS.x + Constants.BORDER_SIZE, Constants.OUTPUT_BOUNDS.y, Constants.OUTPUT_BOUNDS.width / 4, Constants.OUTPUT_BOUNDS.height / 2)),
            ATTACK_2 = new InputOption("DEFAULT_TEXT", new Rectangle(Constants.OUTPUT_BOUNDS.x + Constants.OUTPUT_BOUNDS.width / 2, Constants.OUTPUT_BOUNDS.y, Constants.OUTPUT_BOUNDS.width / 4, Constants.OUTPUT_BOUNDS.height / 2)),
            ATTACK_3 = new InputOption("DEFAULT_TEXT", new Rectangle(Constants.OUTPUT_BOUNDS.x + Constants.BORDER_SIZE, Constants.OUTPUT_BOUNDS.y + Constants.OUTPUT_BOUNDS.height / 2, Constants.OUTPUT_BOUNDS.width / 4, Constants.OUTPUT_BOUNDS.height / 2)),
            ATTACK_4 = new InputOption("DEFAULT_TEXT", new Rectangle(Constants.OUTPUT_BOUNDS.x + Constants.OUTPUT_BOUNDS.width / 2, Constants.OUTPUT_BOUNDS.y + Constants.OUTPUT_BOUNDS.height / 2, Constants.OUTPUT_BOUNDS.width / 4, Constants.OUTPUT_BOUNDS.height / 2));
    /*
     * ATTACK   PKMN
     * ITEM     FLEE
     */
    private final InputOption ATTACK = new InputOption("OPTION_ATTACK", new Rectangle(Constants.OUTPUT_BOUNDS.x + Constants.OUTPUT_BOUNDS.width / 3, Constants.OUTPUT_BOUNDS.y, Constants.OUTPUT_BOUNDS.width / 4, Constants.OUTPUT_BOUNDS.height / 2)),
            PKMN = new InputOption("OPTION_PKMN", new Rectangle(Constants.OUTPUT_BOUNDS.x + 2 * Constants.OUTPUT_BOUNDS.width / 3, Constants.OUTPUT_BOUNDS.y, Constants.OUTPUT_BOUNDS.width / 4, Constants.OUTPUT_BOUNDS.height / 2)),
            ITEM = new InputOption("OPTION_ITEM", new Rectangle(Constants.OUTPUT_BOUNDS.x + Constants.OUTPUT_BOUNDS.width / 3, Constants.OUTPUT_BOUNDS.y + Constants.OUTPUT_BOUNDS.height / 2, Constants.OUTPUT_BOUNDS.width / 4, Constants.OUTPUT_BOUNDS.height / 2)),
            FLEE = new InputOption("OPTION_FLEE", new Rectangle(Constants.OUTPUT_BOUNDS.x + 2 * Constants.OUTPUT_BOUNDS.width / 3, Constants.OUTPUT_BOUNDS.y + Constants.OUTPUT_BOUNDS.height / 2, Constants.OUTPUT_BOUNDS.width / 4, Constants.OUTPUT_BOUNDS.height / 2));
        ...
        if (menuOpen != attackMenuOpen)
        {
            if (menuOpen)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    {
                        attackmenu[i].setText(IO.translate("DEFAULT_TEXT"));
                    }
                }
            ...
            {
                ATTACK_1.setText(IO.translate("DEFAULT_TEXT"));
                ATTACK_2.setText(IO.translate("DEFAULT_TEXT"));
                ATTACK_3.setText(IO.translate("DEFAULT_TEXT"));
                ATTACK_4.setText(IO.translate("DEFAULT_TEXT"));
    ...
    public void sendAttackAction(int index)
    {
        ...
        if (att != null)
        {
            ...
            {
                IO.println(IO.translate("NO_AP"), IO.MessageType.IMPORTANT);
            }
        }
        else
        {
            IO.println(IO.translate("SELECTION_FAILED"), IO.MessageType.IMPORTANT);
        }
    }
    ...
    private void promptPokSelection()
    {
        ...
        new Thread(new Runnable()
        {
            ...
            public void run()
            {
                boolean cancelable = !fight.getPlayersPok().isBsg();
                int index = PokemonMenuPanel.getPokemonSelection(instance, fight.getPlayer(), true, cancelable);
                if (index > Constants.INVALID_INDEX)
                {
                ...
                instance.setAttackMenuOpen(false);
                instance.repaint();
            }
    ...
    public boolean keyPressed(InputConfig.Key key)
    {
        ...
        else
        {
            ...
            {
                Sound.stop();
                if (fight.getPlayersPok().getBasis().EVO_LVL > 0 && fight.getPlayersPok().getLvl() >= fight.getPlayersPok().getBasis().EVO_LVL)
                {
                    utils.Utilities.evolve(GameGrid.getInstance(), fight.getPlayersPok(), PokemonBasis.values()[fight.getPlayersPok().getBasis().ordinal() + 1]);
                }
                else
                {
