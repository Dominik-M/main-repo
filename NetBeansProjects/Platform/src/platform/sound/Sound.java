...
import platform.sound.LineListener.LineEvent;
...
/**
 * 
 * Static Sound Interface. Capable of initializing, storing and playing Soundclips. Supported Soundfile formats are
 * .wav, .mid and .mp3. Note: If you want to use .mp3 files your Main Class has to extend javafx.application.Application
 * and call Application.launch() to init the toolkit. Soundfiles stored in the directory destined by the constant
 * SOUNDFILE are initialized automatically on applications startup. Created 10.03.2016
 * 
...
public class Sound {
	...
	public static final FileFilter soundFileFilter = new FileFilter() {
	...
	public static final File SOUNDFILE = new File(Constants.SOUND_DIRECTORY);
	private static final Dictionary<File, Soundclip> soundclips = new Dictionary<>();
	 ...
	 * 
	 * @param file
	 *        the Soundfile to add
	 * @return true if the soundfile was added to the list or has already been contained.
	 */
	...
	public static boolean isSoundOn() {
		return (boolean) Settings.get("soundOn");
	}
	 ...
	 *        the soundfile.
	 * @param loops
	 *        number of loops. If less than 0 it loops continuously.
	...
	/**
	 * Initializes all compatible soundfiles in the specified sound directory. Supported soundfile formats are .wav and
	 * .mid. Announces the ProgressPanel to display the progress.
	 */
	public static File[] initClips() {
		ProgressPanel.setProgress(0);
		ProgressPanel.setText("Searching Soundfiles...");
		File[] files = SOUNDFILE.listFiles(soundFileFilter);
	...
	public static File[] initClips(File[] files) {
		...
		ProgressPanel.setText(IO.translate("LOADING_SOUNDS"));
		ProgressPanel.setProgress(0);
	...
	public static File[] listSoundFilesInDirectory(File directory) {
		...
		File[] files = directory.listFiles(soundFileFilter);
		return files;
	}

	/**
	 * Initialize a given soundfile. Reads data from the file and stores it in a buffer.
	 * 
	 * @param file
	 *        a soundfile with a supported format. Supported soundfile formats are .wav and .mid.
